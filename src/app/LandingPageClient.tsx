...
const COPY: Record<FloussyLocale, Copy> = {
  ...
  ar: {
    ...
    heroText: "نظّم الدخل ديالك، تتبّع المصاريف، موّل الأهداف، وبقا متحكم ففلوسك بنظام أظرفة واضح، قواعد ذكية، وأتمتة كتعاونك يومياً.",
    ctaMain: "بدا مجاناً",
    ctaMethod: "شوف كيفاش خدام",
    heroMicro: "بلا جداول. سترس أقل. وضوح أكثر فكل درهم.",
    cockpit: "لوحة الميزانية المباشرة",
    problemTitle: "الفلوس كاتدخل… ولكن ماشي ديما باين فين كاتمشي.",
    problemText: "التطبيقات العادية كتوريك شنو وقع من قبل. هنا كتقرر شنو تدير دابا.",
    methodTitle: "طريقة واضحة: كل درهم كيعرف فين غادي.",
    methodText: "كل دخل كيتوزع، كل مصروف كيرتبط بظرف، كل هدف كيتتبع بوحدو، والفائض القابل للادخار يقدر يتحول تلقائياً.",
    stepsTitle: "الميزانية ديالك كتخدم فـ4 خطوات بسيطة.",
    featuresTitle: "كل ما تحتاجو باش تسير الميزانية ديالك يومياً.",
    benefitsTitle: "غموض أقل. تحكم أكثر.",
    compareTitle: "ماشي غير تطبيق تتبع. نظام ميزانية كامل.",
    compareA: "تطبيق تتبع عادي",
    compareB: "7sabek",
    audienceTitle: "مناسب للي باغي يسير فلوسو بمنهج واضح.",
    trustTitle: "منطق مصمم باش ينقص الأخطاء.",
    ...
    finalTitle: "واجد تنظّم فلوسك بوضوح؟",
    ...
    problems: [
      { title: "رؤية ناقصة", text: "كتشوف غير الرصيد العام، ماشي شحال بقى فعلاً فكل جزء." },
      { title: "التجاوز كيبان متأخر", text: "المشكل كيبان غالباً من بعد ما الميزانية تكون تضررات." },
      { title: "خدمة مكررة", text: "التوزيع والتعديل اليدوي كياخذ الوقت كل دورة." },
      { title: "الأهداف كتتنسى", text: "المشاريع المهمة كتتأخر قدام مصاريف اليومي." },
    ],
    solutions: ["أظرفة مصاريف واضحة حسب الحاجة.", "الكاش مفصول قبل التوزيع.", "أهداف مخصصة بتتبع مستقل.", "الديون مفصولة وبأولوية واضحة.", "تحويل الفائض تلقائياً نحو الادخار."],
    steps: [
      ...
      { title: "تتبع المصاريف", text: "كل عملية كتأثر مباشرة على الظرف المرتبط بها." },
      { title: "حسّن نهاية الدورة", text: "الفائض القابل للادخار يقدر يتحول تلقائياً نحو الادخار." },
    ],
    features: ["عمليات ذكية", "ربط التصنيفات بالأظرفة", "توزيع الدخل", "محاكاة قبل التطبيق", "تحويل الفائض تلقائياً", "ترحيل الفائض بتحكم", "أهداف منفصلة", "ديون بأولوية"],
          ...
          <div className="mx-auto w-full max-w-[1200px] px-5 sm:px-8">
            ...
            <div className="mt-8 overflow-hidden rounded-3xl border border-[#e5e7eb] bg-white">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  <tr className="border-t border-[#e5e7eb]"><td className="px-4 py-3">{effectiveLocale === "ar" ? "كيوريك غير الماضي" : "Historical visibility"}</td><td className="px-4 py-3">{effectiveLocale === "ar" ? "لوحة قرار واضحة" : "Decision cockpit"}</td></tr>
                  <tr className="border-t border-[#e5e7eb]"><td className="px-4 py-3">{effectiveLocale === "ar" ? "أتمتة قليلة" : "Low automation"}</td><td className="px-4 py-3">{effectiveLocale === "ar" ? "توزيع، قواعد، وتحويل الفائض" : "Distribution, sweeps, rules"}</td></tr>
                  <tr className="border-t border-[#e5e7eb]"><td className="px-4 py-3">{effectiveLocale === "ar" ? "رصيد واحد فقط" : "Single balance view"}</td><td className="px-4 py-3">{effectiveLocale === "ar" ? "الكاش + الأظرفة + الأهداف + الديون" : "Cash + envelopes + goals + debt"}</td></tr>
                </tbody>
              </table>
            </div>
          </div>
